...
float cambiarFase(vec2 uv, float fase){
  ...
  vec2 puv = pixelate(uv, 32.);
  vec2 cuv = celdate(uv, 5.);
  ...
  vec2 j;
    ...
    j.y = uv.y ;//+ sin(time * .1);
    fondo = fbm(j * 10.);
    fondo = fbm(j * fondo + fase + time * .1);

  if(uv.y > .4){
    //fondo = fbm(uv);
 }else{
   filtro = noise(vec2(uv.x , uv.y * 5.) * 15.);
   float siz = sin(uv.x * PI * 3. * (cos(uv.y + time) * .25 + .5) + time) * .25 + .5;
   siz *= noise(uv + fase + time * .1) * 1.25;
   fondo = subtract(smoothstep(siz, siz + .1, filtro), fondo * (1. +fase) , .25);
 }
  ...
  f1 = 1. - dibujarFigura(p, fondo * noise(uv * 5. + time * .1 + sin(time * .5) * .5 + .5) * .35);
  // f1 = 0.;
  f2 = 0.;
...
void main() {
  ...
  rojo = cambiarFase(uv, 0.25);
  verde = cambiarFase(uv, 0.0);
  azul = cambiarFase(uv, 0.95);
