...
float cambiarFase(vec2 uv, float fase){
  ...
  fondo = random(uv  * sin(time * .5));

  f1 = 1. - dibujarFigura(p, noise(uv* 3. + time * 2. - fase) * .55 + .125);
  f2 = 1. - dibujarFigura(pf, noise(uv* 3. + time * 1.5 + fase) * .4 );

  //f1 = 1. - dibujarFigura(vec2(p.x + baile, p.y), .25);
  //f2 = 1. - dibujarFigura(vec2(p.x - baile, p.y), .25);

  return  (fondo - f1) + f2 * 2.;
}
