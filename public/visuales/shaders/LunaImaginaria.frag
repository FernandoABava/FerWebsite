...
float cambiarFase(vec2 uv, float fase){
  ...
  f1 = 1. - dibujarPoligono(uv, .25, 4.);

  f2 = dibujarFigura(vec2(.5) - cuv, .5);

  fondo = subtract(f2, fondo, fase);
  f1 = subtract(filtro, f1, 1.);

  float factor = 1.;
  //factor = fase;

  return subtract(f1, fondo, factor);
}
